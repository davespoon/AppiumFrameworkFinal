...
public class ADB {
    ...
    public int getAndroidVersion() {
        return Integer.parseInt(getAndroidVersionAsString().
                replace("\\.", ""));
    }
    ...
    public void openAppsActivity(String packageID, String activityID) {
        command("adb -s " + ID + " shell am start -c api.api.android.intent.category.LAUNCHER " +
                "-a api.api.android.intent.action.MAIN -n " + packageID + "/" + activityID);
    }
    ...
    public void forceStopApp(String packageID) {
        command("adb -s " + ID + " shell am force  " + packageID);
    }
    ...
    public Object startLogcat(final String logID, final String grep) {
        ...
        Thread logcat = new Thread(new Runnable() {
            ...
            public void run() {
                if (grep == null) command("adb -s " + ID + " shell logcat -v threadtime > /sdcard/" + logID + ".txt");
                    ...
                    command("adb -s " + ID + " shell logcat -v threadtime | grep -i '" + grep + "'> /sdcard/" + logID +
                            ".txt");
            }
        });
        ...
        else if (pidAfter.size() > 1) throw new RuntimeException("Multiple logcat processes were started when only " +
                "one was expected");
